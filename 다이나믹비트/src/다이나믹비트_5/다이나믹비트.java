...
public class 다이나믹비트 extends JFrame{ //JFrame그래픽기반 라이브러리 
	...
	public 다이나믹비트() {
		...
		startButton.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseEntered(MouseEvent e) {// 마우스가 해당 버튼 위에 올라왔을떄 이벤트
				startButton.setIcon(startButtonEnteredImage);//그림을 바꿔줌
				startButton.setCursor(new Cursor(Cursor.HAND_CURSOR ));//해당 버튼위에 마우스가 올라갔을떄는 손가락 모양으로 바뀜
				Music buttonEnteredMusic = new Music("bbb.mp3", false);//한번만 음악이 실행되게 함
				buttonEnteredMusic.start();
			}
			@Override
			public void mouseExited(MouseEvent e) {//해당 버튼에서 마우스가 나왔을떄 이벤트
				...
				startButton.setCursor(new Cursor(Cursor.DEFAULT_CURSOR ));//해당버튼에서 마우스가 나왔을떄는 원래 상태로 돌아옴
			}
			@Override
			public void mousePressed(MouseEvent e) {//버튼을 클릭했을떄 이벤트
				Music buttonEnteredMusic = new Music("ttt.mp3", false);//한번만 음악이 실행되게 함
		...
		quitButton.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseEntered(MouseEvent e) {// 마우스가 해당 버튼 위에 올라왔을떄 이벤트
				quitButton.setIcon(quitButtonEnteredImage);//그림을 바꿔줌
				quitButton.setCursor(new Cursor(Cursor.HAND_CURSOR ));//해당 버튼위에 마우스가 올라갔을떄는 손가락 모양으로 바뀜
				Music buttonEnteredMusic = new Music("bbb.mp3", false);//한번만 음악이 실행되게 함
				buttonEnteredMusic.start();
			}
			@Override
			public void mouseExited(MouseEvent e) {//해당 버튼에서 마우스가 나왔을떄 이벤트
				...
				quitButton.setCursor(new Cursor(Cursor.DEFAULT_CURSOR ));//해당버튼에서 마우스가 나왔을떄는 원래 상태로 돌아옴
			}
			@Override
			public void mousePressed(MouseEvent e) {//버튼을 클릭했을떄 이벤트
				Music buttonEnteredMusic = new Music("ttt.mp3", false);//한번만 음악이 실행되게 함
